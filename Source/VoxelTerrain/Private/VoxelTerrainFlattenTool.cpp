...
bool FVoxelTerrainFlattenTool::Click() {
	return false;
}

bool FVoxelTerrainFlattenTool::Enter() {
	voxelTerrain = nullptr;
	return true;
}

void FVoxelTerrainFlattenTool::Exit() {
...
bool FVoxelTerrainFlattenTool::MouseMove() {
	return false;
}

bool FVoxelTerrainFlattenTool::InputKey(FViewport * viewport, FKey Key, EInputEvent Event) {
	if (voxelTerrain == nullptr)
		return false;
...
void FVoxelTerrainFlattenTool::Render(FViewport * viewport, const FSceneView * view) {
	//
}

void FVoxelTerrainFlattenTool::Tick(FViewport * viewport, const FSceneView * view, float deltaTime) {
	...
	if (traced && hitResult->GetActor()->IsA<AVoxelTerrain>())
	{
		...
		if (pressed)
		{
			...
			if (!plane)
				plane = new FPlane(center, FVector::UpVector);
...
float FVoxelTerrainFlattenTool::CalculateIncreaseValue(FVector voxelToInvestigate, FVector voxelBrushCenter, float voxelRadius)
{
...
float FVoxelTerrainFlattenTool::CalculateMaxValue(float distance)
{
