...
bool FVoxelTerrainRetopologizeTool::Click() {
	return false;
}

bool FVoxelTerrainRetopologizeTool::Enter() {
	voxelTerrain = nullptr;
	return true;
}

void FVoxelTerrainRetopologizeTool::Exit() {
...
bool FVoxelTerrainRetopologizeTool::MouseMove() {
	return false;
}

bool FVoxelTerrainRetopologizeTool::InputKey(FViewport * viewport, FKey Key, EInputEvent Event) {
	if (voxelTerrain == nullptr)
		return false;
...
void FVoxelTerrainRetopologizeTool::Render(FViewport * viewport, const FSceneView * view) {
	//
}

void FVoxelTerrainRetopologizeTool::Tick(FViewport * viewport, const FSceneView * view, float deltaTime) {
			...
			for (int x = center.X - radius; x < center.X + radius; x++)
			{
				for (int y = center.Y - radius; y < center.Y + radius; y++)
				{
					for (int z = center.Z - radius; z < center.Z + radius; z++)
					{
						...
						if (foundOther && found) {
							if (voxel->Value < 0)
								voxel->Value -= CalculateIncreaseValue(voxelToCheck, center, hitResult->Normal, radius)*deltaTime * 3;
							else
								voxel->Value += CalculateIncreaseValue(voxelToCheck, center, hitResult->Normal, radius)*deltaTime * 3;
							//If <= -1, make -1
							voxel->Value = FMath::Clamp<float>(voxel->Value, -1.f, 1.f);
						}
...
float FVoxelTerrainRetopologizeTool::CalculateIncreaseValue(FVector voxelToInvestigate, FVector voxelBrushCenter, FVector brushNormal, float voxelRadius)
{
